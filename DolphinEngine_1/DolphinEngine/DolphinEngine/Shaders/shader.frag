...
{
	vec3 projCoords = DirectionalLightSpacePos.xyz / DirectionalLightSpacePos.w; //scales to -1 to 1
	projCoords = (projCoords * 0.5) + 0.5; //converts to 0 to 1 range
	...
	shadow /= 9; //Divide by number of pixels 3x3 cube surrounding
...
vec4 CalcSpotLight(SpotLight sLight, int shadowIndex)
{
	...
	if(slFactor > sLight.edge)
	{
		...
		return colour * (1.0f - ((1.0f - slFactor)*((1.0f - 0.0f)/(1.0f - sLight.edge)))); //sets fade around light edges
	}
	else 
	{
	return vec4(0, 0, 0, 0);
